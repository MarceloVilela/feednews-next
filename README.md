...
- **Renderização híbrida**: páginas estáticas por origem via `generateStaticParams`/ISR (revalidação a cada 24h), com os dados reais do feed buscados direto no servidor (Server Components) — sem fetch client-side, evitando scraping síncrono no build.
