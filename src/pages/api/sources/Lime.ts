...
class Lime implements ISource {
  ...
  async parseResults(document: Document) {
    ...
    const contents = [...document.querySelectorAll('table tr')]
      ...
      .filter(item => item.link.includes('.torrent') && !item.desc_link.includes('undefined') && !isNaN(item.seeds) && !isNaN(item.leech))
