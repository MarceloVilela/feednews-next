...
class Vt implements ISource {
  getOriginUrl(): string {
    return 'https://vingadorestorrent.com.br';
  }

  async detail({ url }: IShowDetailMagnetDTO): Promise<Answer> {
    //const response = await JSDOM.fromURL(url);
    const response = await JSDOM.fromFile('./limon-detail.html');

    const { document } = response.window;
    //fs.writeFileSync("./limon-detail.html", String(document.querySelector('html')?.innerHTML));
    
    const name = document.querySelector('#main img')?.getAttribute('title');
    const desc_link = document.querySelector('meta[name="generator"]+link[rel="canonical"]')?.getAttribute('href');
    const thumb = document.querySelector('#main img')?.getAttribute('src');
  ...
  async search({ search_query }: ISearchParams): Promise<Answer[]> {
    const url = `${this.getOriginUrl()}/search?q=${search_query}`
    console.table(['url', url]);
    const response = await JSDOM.fromURL(url);
    //const response = await JSDOM.fromFile('./limon.html');

    const { document } = response.window;
    fs.writeFileSync("./vt.html", String(document.querySelector('html')?.innerHTML));
    return []
    
    const results = await this.parseResults(document, '.movies-list .item');
