...
class Ft implements ISource {
  getOriginUrl(): string {
    return 'https://filmestorrent.me';
  }

  async detail({ url }: IShowDetailMagnetDTO): Promise<Answer> {
    //const response = await JSDOM.fromURL(url);
    const response = await JSDOM.fromFile('./ft-detail.html');

    const { document } = response.window;
    //fs.writeFileSync("./ft-detail.html", String(document.querySelector('html')?.innerHTML));
    
    const name = document.querySelector('#main img')?.getAttribute('title');
    const desc_link = document.querySelector('link[rel="canonical"]')?.getAttribute('href');
  ...
  async parseResults(document: Document, selector: string) {
    ...
    const getContent = async (art: Element) => {
      ...
      return {
        ...
        desc_link: String(art.querySelector('a.more-link')?.getAttribute('href')),
      }
    };
  ...
  async search({ search_query }: ISearchParams): Promise<Answer[]> {
    ...
    //const response = await JSDOM.fromURL(url);
    const response = await JSDOM.fromFile('./ft.html');

    const { document } = response.window;
    //fs.writeFileSync("./ft.html", String(document.querySelector('html')?.innerHTML));
    
    const results = await this.parseResults(document, '#main .entry-content');
