...
class Limon implements ISource {
  getOriginUrl(): string {
    return 'https://limontorrents.com';
  }
  ...
  async search({ search_query }: ISearchParams): Promise<Answer[]> {
    ...
    const { document } = response.window;
    //fs.writeFileSync("./limon.html", String(document.querySelector('html')?.innerHTML));
