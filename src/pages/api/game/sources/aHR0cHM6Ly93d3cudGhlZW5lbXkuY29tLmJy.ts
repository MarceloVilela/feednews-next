...
class Th33n implements ISource {
  getOriginUrl(): string {
    return atob("aHR0cHM6Ly93d3cudGhlZW5lbXkuY29tLmJy");
  }
  ...
  async getHome(): Promise<IResponseHomeDTO> {
    ...
    const getContent = (elPost: Element) => {
      return {
        ...
        title: replaceSpaces(
          String(
            elPost.querySelector("a.news-list__item__content__title")
              ?.textContent,
          ),
        ),
        thumb: addHttp(
          String(elPost.querySelector("img")?.getAttribute("src")),
        ),
        created_at: replaceSpaces(
          String(
            elPost.querySelector(".news-list__item__content__info__time span")
              ?.textContent,
          ),
        ),
      };
    ...
    const getDataContent = (elPost: Element) => {
      return {
        ...
        title: replaceSpaces(
          String(elPost.querySelector("p.card__title")?.textContent),
        ),
        thumb: addHttp(
          String(elPost.querySelector(".card__image img")?.getAttribute("src")),
        ),
        created_at: replaceSpaces(
          String(
            elPost.querySelector(".news-list__item__content__info__time span")
  ...
  /*async detail({ url }: IShowDetailMagnetDTO): Promise<Answer> {
    ...
    const getLinks = (link: Element) => {
      ...
      text = !text
        ? String(
            new URLSearchParams(String(link.getAttribute("href"))).get("dn"),
          )
        : text;
  ...
  async parseResults(document: Document, selector: string) {
    const getContent = async (art: Element) => {
      ...
      return {
        name: String(
          art
            .querySelector("h2.entry-title a")
            ?.textContent?.replace(/\n|\r|\t/g, "")
            ?.replace(/\\n|\\r|\\t/g, "")
            ?.replace(/\s{2,}/g, ""),
        ),
  ...
  async top(): Promise<ITrendDTO> {
    ...
    const top = await this.parseResultsAside(
      document,
      "#widgets-wrap-sidebar-left .textwidget>p",
    );
