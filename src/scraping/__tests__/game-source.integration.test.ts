import { GET } from "app/api/game/source/route";
import { Post, sources } from "../game";
...
describe("Game Source API Integration Tests", () => {
  ...
  it.each(sourcesData)(
    ...
    async ({ url }) => {
      const response = await callRoute(
        "http://localhost/api/game/source?url=" + encodeURIComponent(url),
      );
  ...
  it("should return a structured error when the URL parameter is missing", async () => {
    const response = await callRoute("http://localhost/api/game/source");
  ...
  it("should return a structured error when the alias is unknown", async () => {
    const response = await callRoute(
      "http://localhost/api/game/source?url=nonexistent-site.com",
    );
  ...
  it("should return a structured error when the URL parameter is repeated", async () => {
    const response = await callRoute(
      "http://localhost/api/game/source?url=a&url=b",
    );
