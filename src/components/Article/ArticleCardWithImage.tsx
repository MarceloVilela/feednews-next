...
export default function ArticleCardWithImage({articles}: ArticleCardWithImageProps){
  ...
  return (
    ...
    {_articles.map(({title, link, id, thumb}, key) => (
      <div
        key={id}
        className="grid grid-cols-2 flex- rounded-lg bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700">
        <a 
          href={link} 
          target="_blank"
          rel="noreferrer"
        ...
        <div className="flex flex-1 items-center">
          ...
          <p className="mx-2 text-base text-neutral-600 dark:text-neutral-200">
            {title}
          </p>
          <button
            type="button"
